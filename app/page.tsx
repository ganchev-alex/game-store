import PrimeCarousel from "@/components/home_page_components/primary_carousel/PrimeCarousel";

import ICarouselDataSlot from "../utility/interfaces/ICarouselDataSlot";

const carouselData: ICarouselDataSlot[] = [
  {
    gameTitle: "Forza Horizon 4",
    titleLogoUrl:
      "https://swissgameguides.com/maps/forza_horizon_4/world/media/web/logo.png",
    thumb: "https://wallpapercave.com/wp/wp4748912.jpg",
    stateTag: "Now Available",
    description:
      "Forza Horizon 4 is a racing video game set in an open world environment based in a fictionalised Great Britain. It focuses specifically on casual street racing, rather than professionally on race tracks, as it takes place on a map of temporarily closed public roads.",
    price: 69.99,
  },
  {
    gameTitle: "Apex Legends",
    titleLogoUrl:
      "https://preview.redd.it/1kxnu79d9q841.png?auto=webp&s=5cb2f2f0a20c5c8bdaaaf462ee91bf64bdeb1e21",
    thumb: "https://images3.alphacoders.com/115/thumb-1920-1153033.jpg",
    stateTag: "Free To Play",
    description:
      "FPS battle royale, your goal in Apex Legends is to be the last team standing. Each match, 20 teams of three drop from a ship along a randomized flight path down to the map.",
    price: 0,
  },
  {
    gameTitle: "Destiny 2",
    titleLogoUrl:
      "https://cdn2.steamgriddb.com/logo_thumb/706e490fedac1d732f20dc8c18f649df.png",
    thumb: "https://images.alphacoders.com/136/thumb-1920-1364600.jpeg",
    stateTag: "New DLC Available",
    description:
      "Destiny 2 is a first-person shooter game that incorporates role-playing and massively multiplayer online game (MMO) elements.",
    price: 59.99,
  },
  {
    gameTitle: "God of War",
    titleLogoUrl:
      "https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/92554480-bde6-4f5c-9861-7022237da1ce/dfk2tos-4299426f-c415-41ad-becf-cae3763ac48d.png/v1/fill/w_1024,h_284/god_of_war_2018_logo_by_brsbr_dfk2tos-fullview.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..MVMNs_v2I-DeHgKSo8lUt7i3yh56On-52FOiEQ2JRh0",
    thumb:
      "https://wallpapers.com/images/hd/incredible-action-god-of-war-2i0z7fl3dbkhpfws.jpg",
    stateTag: "Now On Sale",
    description:
      "In God of War, players control Kratos, a Spartan warrior who is sent by the Greek gods to kill Ares, the god of war. As the story progresses, Kratos is revealed to be Ares' former servant, who had been tricked into killing his own family and is haunted by terrible nightmares.",
    price: 51.99,
    discount: 15,
  },
  {
    gameTitle: "Elder Ring",
    titleLogoUrl:
      "https://image.api.playstation.com/vulcan/img/rnd/202108/0523/qa8tlmB0UGRJa8mS3dS8hUoU.png",
    thumb:
      "https://www.pcgamesn.com/wp-content/sites/pcgamesn/2022/01/Elden-Ring-bosses-all-boss-list.jpg",
    stateTag: "Recently Updated",
    description:
      "Elden Ring is an action RPG which takes place in the Lands Between, sometime after the Shattering of the titular Elden Ring. ",
    price: 59.99,
  },
];

export default function Home() {
  return (
    <>
      <PrimeCarousel carouselData={carouselData} />
    </>
  );
}
